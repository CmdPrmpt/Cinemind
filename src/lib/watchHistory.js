...
async function getWatchHistory(config, limit, targetType, inputMode, services = {}) {
    ...
    if (librarySource === 'nuvio') {
        const getNuvioWatchHistory = services.getNuvioWatchHistory ||
            require('../services/nuvio').getNuvioWatchHistory;
        return getNuvioWatchHistory(config.nuvioAccessToken, config.nuvioRefreshToken || '', limit, targetType, inputMode, config.nuvioProfileId || 1);
    }

    const getUserWatchHistory = services.getUserWatchHistory ||
        require('../services/stremio').getUserWatchHistory;
    // Use profile-specific authKey if a Stremio profile is selected
    const stremioAuthKey = config.stremioProfileAuthKey || config.authKey;
    return getUserWatchHistory(stremioAuthKey, limit, targetType, inputMode);
}
