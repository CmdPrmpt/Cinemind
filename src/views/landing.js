...
<body>
                    ...
                    <div class="glass-card">
                        <div class="form-group">
                            ...
                            <div class="tabs" id="librarySourceTabs">
                                ...
                                <div class="tab" onclick="switchLibrarySource('nuvio')">Nuvio</div>
                            </div>
                        </div>
                        ...
                        <div id="stremio-auth">
                            ...
                            <!-- Stremio Profile Section -->
                            <div id="stremioProfileSection" style="display:none; margin-top:16px;">
                                <div class="form-group">
                                    <label class="label">
                                        Stremio Profile
                                        <div class="tooltip-container"><span class="tooltip-icon">?</span>
                                            <div class="tooltip-content"><span class="tooltip-header">Stremio Profiles</span>
                                            Stremio allows multiple profiles per account. Select which profile's library to use.<br><br>
                                            If your profile has a PIN, you'll be prompted to enter it.
                                            </div>
                                        </div>
                                    </label>
                                    <div class="select-wrapper">
                                        <select id="stremioProfileId" class="select">
                                            <option value="">Main Account (Default)</option>
                                        </select>
                                        <div class="select-arrow">▼</div>
                                    </div>
                                    <div id="stremioProfileMsg" class="msg" style="font-size:0.85em;"></div>
                                </div>
                        ...
                        <!-- Nuvio Auth Section -->
                        <div id="nuvio-auth" style="display:none">
                            <div class="grid-2">
                                <div class="form-group"><label class="label">Email</label><input type="email" id="nuvioEmail" class="input" placeholder="your@email.com"></div>
                                <div class="form-group"><label class="label">Password</label><input type="password" id="nuvioPass" class="input" placeholder="Your Nuvio password"></div>
                            </div>
                            <button id="nuvioLoginBtn" class="btn-primary" style="margin-top:10px">Sign In</button>
                            <div id="nuvioMsg" class="msg" style="text-align:center"></div>
                            <div id="nuvioProfileSection" style="display:none; margin-top:16px;">
                                <div class="form-group">
                                    <label class="label">Profile</label>
                                    <div class="select-wrapper">
                                        <select id="nuvioProfileId" class="select">
                            ...
                            <small style="color:var(--text-muted); margin-top:8px; display:block; text-align:center;">Don't have an account? <a href="https://nuvio.tv" target="_blank" style="color:#818cf8;">Sign up on Nuvio</a></small>
                            <input type="hidden" id="nuvioAccessToken" value="">
                        </div>
                    </div>
                     ...
                     <div class="glass-card">
                        ...
                        <div style="margin-top:40px; border-top:1px solid var(--border); padding-top:24px;">
                            <label class="label">
                                ...
                                <div class="tooltip-container"><span class="tooltip-icon">?</span>
                                    <div class="tooltip-content"><span class="tooltip-header">Password-Protected Keys</span>
                                    Your API keys are encrypted with your password (PBKDF2 + AES-256). Your password is <b>never stored</b> - it's required to unlock your keys.<br><br>
                                    <b>Requirements:</b><br>
                                    • 8+ characters<br>
        ...
        <div id="installScreen">
             ...
             <div class="container animate-in" style="max-width:600px; padding-top:0; position:relative; z-index:10;">
                 ...
                 <div style="display:flex; gap:12px; justify-content:center; flex-wrap:wrap;">
                     ...
                     <button class="btn-secondary" id="deleteConfigBtn" onclick="deleteConfig()" style="color:#f87171;">🗑️ Delete Config</button>
                 </div>
             </div>
        </div>
    ...
    <script>
        ...
        function switchLibrarySource(source) {
            ...
            document.getElementById('nuvio-auth').style.display = 'none';
            ...
            if(source === 'stremio') {
                ...
                document.getElementById('nuvio-auth').style.display = 'block';
            }
            updateSummary();
        }
        ...
        function updateSummary() {
             ...
             const libLabels = { stremio: 'Stremio', mdblist: 'MDBlist', nuvio: 'Nuvio' };
             document.getElementById('sumLibrarySource').textContent = libLabels[libSource] || 'Stremio';
        ...
        async function checkKey(key) {
             ...
             try {
                ...
                if(d.valid) {
                    ...
                    updateSidebar(true);
                    // Fetch Stremio profiles after successful key validation
                    await fetchStremioProfiles(key);
                    return true;
                }
        ...
        document.getElementById('nuvioLoginBtn').onclick = async () => {
             ...
             const password = document.getElementById('nuvioPass').value.trim();
             const msg = document.getElementById('nuvioMsg');
             if(!email || !password) {
                 msg.textContent = 'Email and password required'; msg.className = 'msg show error';
                 return;
             }
             
             msg.textContent = "Signing in..."; msg.className = "msg show";
             try {
                 ...
                 const data = await res.json();
                 if (data.success) {
                     document.getElementById('nuvioAccessToken').value = data.access_token;
                     // Store refresh token in a data attribute for later use
                     document.getElementById('nuvioAccessToken').setAttribute('data-refresh-token', data.refresh_token || '');
                     msg.textContent = '✓ Logged In';
                     msg.className = 'msg show success';
                     updateSidebar(true);
                     
                     // Fetch profiles
                     await fetchNuvioProfiles(data.access_token, data.refresh_token);
                 } else {
                     msg.textContent = data.error || 'Login Failed'; msg.className = 'msg show error';
                 }
             } catch(e) {
                 msg.textContent = 'Network Error'; msg.className = 'msg show error';
             }
        };
        
        async function fetchNuvioProfiles(accessToken, refreshToken) {
            ...
            try {
                const res = await fetch('/api/nuvio-profiles', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ nuvioAccessToken: accessToken, nuvioRefreshToken: refreshToken || '' })
                });
                const data = await res.json();
                if (data.success && data.profiles.length > 0) {
                    ...
                    data.profiles.forEach(p => {
                        ...
                        opt.value = p.profile_index;
                        opt.textContent = p.name + ' (Profile ' + p.profile_index + ')';
                        select.appendChild(opt);
                    });
                    section.style.display = 'block';
                }
            } catch(e) {
                console.error('Failed to fetch Nuvio profiles:', e);
            }
        }

        async function fetchStremioProfiles(authKey) {
            const select = document.getElementById('stremioProfileId');
            const section = document.getElementById('stremioProfileSection');
            const msg = document.getElementById('stremioProfileMsg');
            const profileAuthKey = document.getElementById('stremioProfileAuthKey');
            try {
                ...
                if (data.success && data.profiles && data.profiles.length > 0) {
                    select.innerHTML = '<option value="">Main Account (Default)</option>';
                    data.profiles.forEach(p => {
                        const opt = document.createElement('option');
                        opt.value = p.id;
                        opt.textContent = p.name + (p.hasPin ? ' 🔒' : '');
                        opt.dataset.hasPin = p.hasPin ? 'true' : 'false';
                        select.appendChild(opt);
                    });
                    section.style.display = 'block';
                    msg.textContent = '';
                    msg.className = 'msg';
                    // Reset profile auth key when profiles refresh
                    profileAuthKey.value = '';
                } else if (data.success && data.singleUser) {
                    section.style.display = 'none';
                    profileAuthKey.value = '';
                    msg.textContent = '';
                    msg.className = 'msg';
                } else {
                    msg.textContent = data.error || 'Failed to load profiles';
                    msg.className = 'msg show error';
                }
            } catch(e) {
                console.error('Failed to fetch Stremio profiles:', e);
                msg.textContent = 'Network error fetching profiles';
                msg.className = 'msg show error';
            }
        }

        async function authenticateStremioProfileSelection(authKey, profileId, hasPin) {
            const msg = document.getElementById('stremioProfileMsg');
            ...
            if (hasPin) {
                ...
                if (pin === null) {
                    msg.textContent = 'Profile authentication cancelled';
                    msg.className = 'msg show warning';
                    return false;
                }
            }

            msg.textContent = 'Authenticating profile...';
            msg.className = 'msg show';
            try {
                const res = await fetch('/api/stremio-authenticate-profile', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ authKey, profileId, pin })
                });
                const data = await res.json();
                if (data.success && data.authKey) {
                    profileAuthKeyInput.value = data.authKey;
                    msg.textContent = '✓ Profile authenticated';
                    msg.className = 'msg show success';
                    return true;
                } else {
                ...
                console.error('Failed to authenticate Stremio profile:', e);
                msg.textContent = 'Network error during authentication';
                msg.className = 'msg show error';
                return false;
            }
        }
        ...
        document.getElementById('loginBtn').onclick = async () => {
             ...
             try {
                 ...
                 if(data.result && data.result.authKey) {
                     ...
                     updateSidebar(true);
                     // Fetch Stremio profiles after login
                     await fetchStremioProfiles(data.result.authKey);
                 } else {
        ...
        // Stremio profile selection handler
        document.getElementById('stremioProfileId').onchange = async function() {
            ...
            // Reset profile auth key on change
            profileAuthKeyInput.value = '';

            if (!profileId) {
                // User selected "Main Account"
                msg.textContent = 'Using main account auth key';
                msg.className = 'msg show';
                return;
            }
            ...
            await authenticateStremioProfileSelection(authKey, profileId, hasPin);
        };
        ...
        document.getElementById('generateBtn').onclick = async () => {
             ...
             } else if (librarySource === 'nuvio') {
                 const nuvioToken = document.getElementById('nuvioAccessToken').value.trim();
                 if(!nuvioToken) { alert("Please sign in with your Nuvio account first"); btn.disabled = false; btn.textContent = btn.dataset.originalText || "Generate Addon"; return; }
             }
             ...
             const config = {
                 ...
                 nuvioAccessToken: document.getElementById('nuvioAccessToken').value.trim(),
                 nuvioRefreshToken: document.getElementById('nuvioAccessToken').getAttribute('data-refresh-token') || '',
                 nuvioProfileId: parseInt(document.getElementById('nuvioProfileId').value) || 1,
                 stremioProfileId: document.getElementById('stremioProfileId').value || '',
                 stremioProfileAuthKey: document.getElementById('stremioProfileAuthKey').value || ''
             };
             ...
             try {
                ...
                if(data.token) {
                    ...
                    // Save to DB for short UUID URL.
                    // Reuse the existing UUID when editing so the install URL stays stable.
                    const priorUuid = document.getElementById('urlDisplay').getAttribute('data-uuid') || undefined;
                    let currentUuid = null;
                    try {
                        const saveRes = await fetch('/api/save-config', {
                            method: 'POST', headers: {'Content-Type': 'application/json'},
                            body: JSON.stringify({ encryptedToken: data.token, uuid: priorUuid })
                        });
                        const saveData = await saveRes.json();
                        if (saveData.uuid) currentUuid = saveData.uuid;
                    } catch (e) { /* fall back to long URL */ }
                    
                    // Store for later use by delete/long-url functions
                    if (currentUuid) {
                        document.getElementById('urlDisplay').setAttribute('data-uuid', currentUuid);
                    }
                    document.getElementById('urlDisplay').setAttribute('data-token', data.token);
                    document.getElementById('urlDisplay').setAttribute('data-encrypted-password', data.encryptedPassword || '');
                    
                    let url, stremioUrl;
                    if (currentUuid) {
                        // Short UUID URL
                        if (data.encryptedPassword) {
                            url = window.location.protocol + '//' + window.location.host + '/' + currentUuid + '/' + encodeURIComponent(data.encryptedPassword) + '/manifest.json';
                            stremioUrl = 'stremio://' + window.location.host + '/' + currentUuid + '/' + encodeURIComponent(data.encryptedPassword) + '/manifest.json';
                        } else {
                            url = window.location.protocol + '//' + window.location.host + '/' + currentUuid + '/manifest.json';
                            stremioUrl = 'stremio://' + window.location.host + '/' + currentUuid + '/manifest.json';
                        }
                    } else {
                        // Fall back to long token URL
                        if (data.encryptedPassword) {
                    ...
                    // Show password reminder if protected (remove any prior one so repeated
                    // edit/save cycles don't stack duplicate notices)
                    const existingReminder = document.getElementById('passwordReminder');
                    if (existingReminder) existingReminder.remove();
                    if (data.passwordProtected) {
                        const reminderEl = document.createElement('p');
                        reminderEl.id = 'passwordReminder';
        ...
        async function deleteConfig() {
            ...
            if (!confirm('Are you sure you want to delete this configuration? The short URL will stop working immediately.')) return;
            
            try {
                const res = await fetch('/api/delete-config/' + encodeURIComponent(uuid), { method: 'DELETE' });
                const data = await res.json();
                if (data.success) {
                    document.getElementById('deleteConfigBtn').textContent = '✓ Deleted';
                ...
                try {
                    ...
                    if (config.catalog_order && Array.isArray(config.catalog_order)) {
                        ...
                        config.catalog_order.forEach(cat => {
                            ...
                            // Generated configs omit the enabled flag (catalog_order lists only enabled
                            // catalogs), so treat presence as enabled unless explicitly disabled.
                            const enabled = typeof cat === 'string' ? true : (cat.enabled !== false);
        ...
        function getCatalogGenres(id) {
            ...
            if (id.includes('ser') || id.includes('series') || id.includes('show')) return GENRES.TMDB_TV;
        ...
        function populateForm(cfg) {
             const hasConfig = cfg && (cfg.authKey || cfg.mdblistApiKey || cfg.nuvioAccessToken);

             // If this configure page was opened from a stored UUID URL, remember the UUID so
             // saving updates that config in place and keeps the install URL stable.
             const firstSeg = window.location.pathname.split('/').filter(p => p)[0];
             const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
             if (firstSeg && UUID_RE.test(firstSeg)) {
                 document.getElementById('urlDisplay').setAttribute('data-uuid', firstSeg);
             }
             ...
             if(hasConfig) {
                 ...
                 if(cfg.nuvioAccessToken) {
                     document.getElementById('nuvioAccessToken').value = cfg.nuvioAccessToken;
                     if (cfg.nuvioRefreshToken) document.getElementById('nuvioAccessToken').setAttribute('data-refresh-token', cfg.nuvioRefreshToken);
                     // Fetch profiles to populate the dropdown
                     fetchNuvioProfiles(cfg.nuvioAccessToken, cfg.nuvioRefreshToken || '');
                 }
                 if(cfg.nuvioProfileId) document.getElementById('nuvioProfileId').value = cfg.nuvioProfileId;
                 
                 // Restore Stremio profile — fetch profiles first, then set selection and authKey
                 if (cfg.librarySource === 'stremio' && cfg.authKey) {
                     fetchStremioProfiles(cfg.authKey).then(() => {
                         if (cfg.stremioProfileId) {
                             document.getElementById('stremioProfileId').value = cfg.stremioProfileId;
                         }
                         if (cfg.stremioProfileAuthKey) {
                             document.getElementById('stremioProfileAuthKey').value = cfg.stremioProfileAuthKey;
                         }
                     });
                 }
        ...
        async function unlockConfig() {
            ...
            try {
                ...
                // Configure URLs are either /TOKEN/configure or /TOKEN/PASSWORD/configure.
                const token = cleanParts[0];
