...
// Map the manifest catalog id (e.g. 'personalized_recs_movies') back to the short
// config key (e.g. 'std_mov') used in config.catalog_order entries.
const MANIFEST_ID_TO_KEY = Object.create(null);
for (const [shortKey, def] of Object.entries(CATALOG_DEFINITIONS)) {
    MANIFEST_ID_TO_KEY[def.id] = shortKey;
}
...
function createAddonInterface(config) {
    ...
    const librarySourceLabel = config.librarySource === 'mdblist' ? 'MDBList' : config.librarySource === 'nuvio' ? 'Nuvio' : 'Stremio';
    ...
    builder.defineCatalogHandler(async ({ type, id, extra }) => {
        ...
        // config.catalog_order entries are keyed by the short key (e.g. 'std_mov'),
        // but Stremio sends the manifest id (e.g. 'personalized_recs_movies').
        const catalogKey = MANIFEST_ID_TO_KEY[id] || id;

        // Derive exclusions for THIS catalog so changing them invalidates the cache
        let catalogExclusions = [];
        if (Array.isArray(config.catalog_order)) {
            const cc = config.catalog_order.find(c => c && typeof c === 'object' && (c.id === catalogKey || c.catalogId === catalogKey));
            if (cc && Array.isArray(cc.excludedGenres)) catalogExclusions = cc.excludedGenres;
        }
        ...
        const settingsHash = crypto.createHash('sha256').update(JSON.stringify({
            ...
            excl: catalogExclusions
        })).digest('hex');

        const cacheKeyId = config.librarySource === 'mdblist' ? config.mdblistApiKey : config.librarySource === 'nuvio' ? crypto.createHash('sha256').update(`${config.nuvioAccessToken}:${config.nuvioProfileId || 1}`).digest('hex').slice(0, 16) : crypto.createHash('sha256').update(`${config.authKey}:${config.stremioProfileId || ''}:${config.stremioProfileAuthKey || ''}`).digest('hex').slice(0, 16);
        ...
        const runUpdate = async () => {
            ...
            try {
                ...
                if (selectedItems.length > 0) {
                    const result = await generateCatalog(config, type, requiredGenreId, allLibraryIds, selectedItems, isAnimeCatalog, strategy, catalogKey);
