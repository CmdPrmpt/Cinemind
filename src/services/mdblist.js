...
async function getMDBlistDebugHistory(apiKey) {
    try {
        const [watchedRes, playbackRes] = await Promise.all([
            axios.get(`${MDBLIST_API_URL}/sync/watched`, {
                params: { apikey: apiKey, limit: 5000 },
        ...
        const episodes = watchedData.episodes || [];
        const moviesWatched = movies.length;
        const episodesWatched = episodes.length;
        const inProgress = playbackData.length;

        // Aggregate unique shows from episodes
        ...
        return {
            total_movies_watched: moviesWatched,
            total_episodes_watched: episodesWatched,
            unique_shows: showIds.size,
            in_progress: inProgress,
            sample_movies: movies.slice(0, 5).map(m => ({
                title: (m.movie || {}).title || 'Unknown',
                id: ((m.movie || {}).ids || {}).imdb || ((m.movie || {}).ids || {}).tmdb || 'N/A',
                last_watched: m.last_watched_at || 'N/A'
            })),
            sample_episodes: episodes.slice(0, 10).map(e => ({
                show: ((e.episode || {}).show || {}).title || 'Unknown',
                season: (e.episode || {}).season || 0,
                episode: (e.episode || {}).number || 0,
                id: (((e.episode || {}).show || {}).ids || {}).imdb || (((e.episode || {}).show || {}).ids || {}).tmdb || 'N/A',
                last_watched: e.last_watched_at || 'N/A'
            })),
            sample_in_progress: playbackData.slice(0, 5).map(p => ({
                title: p.title || 'Unknown',
                type: p.type || 'N/A',
                progress: p.progress || '0',
                id: (p.ids || {}).imdb || (p.ids || {}).tmdb || 'N/A',
                paused_at: p.paused_at || 'N/A'
            }))
        };
    } catch (error) {
        console.error('[MDBlist] Debug fetch failed:', error.message);
